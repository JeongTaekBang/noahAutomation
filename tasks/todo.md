...
생겼고, 9/3분은 DN에 있는데 9/11분을 돌리면 "기록할 게 없다"고 뜬다.

- [x] 원인 특정 — 9/11분이 '이미 입력됨(다른 출고일)'로 건너뜀. 근거로 쓴 9/3 기록은
      같은 목록의 9/3 출고 자신의 것이었다 (균등 분할이라 라인·수량이 같다)
- [x] 실데이터 가드 발동 전수 분류 (P03~P09 5건): 진짜 중복 4건 / 오탐 1건(0868)
- [x] 회귀 테스트 4건 먼저 작성 → 수정 전 코드에서 3건 실패(버그 재현)·1건 통과(안전장치) 확인
- [x] `_billed_beyond_dn` — PO Status 계산서 수량 − DN 수량으로 "다른 회차의 기록"을 가른다
- [x] 실데이터 수정 전/후 대조 — 전체 실행 차이 0868 1건뿐, 증분 재현 658건 차이 0건
- [x] 전체 **909 passed, 2 skipped**
- [x] CLAUDE.md / docs/CHANGELOG.md / tasks/lessons.md 반영

## 리뷰

`create_dn.py P09`를 다시 돌리면 `SOD-2026-0868` 9/11 L1 20개가 추가 대상으로 뜬다
(워크북 쓰기는 사용자의 y/N 몫 — 이번 작업에서 마스터 파일은 건드리지 않았다).

곁가지로 본 것: P06을 실제 DN 상태로 다시 돌리면 `SOD-2026-0301` 6/30 출고 3라인이 추가
대상으로 뜬다. 사람이 그 출고를 7/1·7/20 두 번으로 나눠 적어 둬서(노아 보관분 72개가 7/20에
나감) 어느 한 날짜와도 조합이 안 맞기 때문이다. 이번 수정과 무관한 기존 한계이고, 지난 기간을
다시 돌릴 때만 문제가 된다.
