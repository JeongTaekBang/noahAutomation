...
- **OC 메일은 해외 전용이고, 조인키가 사업자번호가 아니라 고객코드다.** `SO_해외`의
  ...
  거래명세표와 동일하다
- **거래명세표 하단 PO No. 옆에 호선명을 병기하는 건 조선 기자재 4사뿐이다**
  (`config.TS_PO_REMARK_CUSTOMERS` = 스칸텍·브이티엘·엔이에스·파나시아, 고객명 부분일치 —
  2026-08-25 사용자 지시). 호선명은 `SO_국내.Remarks`에 있고 발주번호별 괄호 병기다:
  `SCT2605-134 (한화-H4394, ...)`. 다른 거래처는 Remarks에 내부 메모가 있어 붙이면 안 된다.
