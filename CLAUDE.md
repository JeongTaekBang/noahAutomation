...
python create_ci.py DNO-2026-0001        # Commercial invoice (라인별 HS CODE, SECTORIEL은 고객용 한 장 더)
...
- **OneDrive로 공유하는 SQLite는 원본을 열지 않는다** (`dashboard.py: _db_snapshot`): DB는 `journal_mode=wal`이라 최신 커밋이 `-wal` 사이드카에 먼저 들어가는데, OneDrive는 본체와 사이드카를 **각각 따로** 동기화한다. 원본을 직접 열면 (1) 커밋이 빠진 상태를 보거나 (2) 읽기 잠금 때문에 OneDrive가 파일을 교체 못 해 **"충돌된 사본"** 이 생긴다 — 둘 다 "사람마다 숫자가 다르다"로 뒤늦게 드러난다. 그래서 백업 API로 일관된 사본을 떠서 그것만 읽고, 캐시 키가 `(mtime, size)`라 원본이 갱신되면 자동으로 다시 뜬다. 발행 쪽은 `sync_db.py: checkpoint_wal()`이 `PRAGMA wal_checkpoint(TRUNCATE)`로 `-wal`을 비워 **단일 파일로 완결**시킨다. **단, 사본을 읽는 것은 읽기에 한한다 — 쓰기는 반드시 원본으로** (`dashboard.py: _write_conn`). 사본은 `os.replace()`로 **같은 경로를 통째로 갈아치우는** 일회용이라 거기 쓴 것은 다음 재복사(원본 갱신 또는 대시보드 재시작) 때 흔적 없이 사라진다 — 예외가 안 나므로 화면에서는 성공한 것처럼 보인다. 실제로 스냅샷 도입(2026-07-31) 직후부터 '확인 완료'(`_ack_so_change`)가 사본에 쓰이는 바람에 ack이 매번 증발했고, 눌러도 다음 실행에 같은 목록이 되돌아왔다 (2026-09-07 발각, 한 달간 조용히 유실). 쓰기 뒤에는 `wal_checkpoint(TRUNCATE)` + `_db_snapshot.clear()`로 사본을 다시 뜬다. `tests/test_dashboard.py: TestAckPersistence`가 감시.
- **대사 요약과 상세는 총액이 같아야 한다** (`reconcile_po.py: _build_excel_vs_ax`): `요약` 시트는 Service를 **잔차**(GRN 총액 − 국내 − 해외)로 잡으므로 AX 합계가 정의상 **GRN 총액**이고, 한 AX PO가 국내/해외 양쪽 키에 걸리면 **국내 우선**으로 한 버킷에만 귀속시킨다. `Excel_vs_AX` 상세도 같은 우선순위로 GRN을 **AX PO당 1행에만** 붙여야 한다 — 예전엔 AX PO로 outer merge해서, 같은 AX PO가 Product와 Service 양쪽에 있으면 GRN 1건이 두 행에 복제돼 AX 합계가 부풀었다 (2026-08 실측 `P024792`: 두 시트 Diff 총액이 2,530,020 어긋남). **두 시트가 어긋나면 사용자는 어느 쪽을 믿을지 알 수 없다.** 덧붙여 **같은 AX PO가 두 분류에 걸리는 것 자체가 AX PO 오타 신호**라 경고로 남긴다 (그 오타의 짝은 어느 Product 키에도 안 잡혀 Service 잔차로 숨는데, `_all_product_ax` 가드는 PO 시트에 존재하는 번호만 보므로 **오타 번호는 그 가드에 안 걸린다**). `tests/test_reconcile_po.py`가 감시.
...
- **OC 메일은 해외 전용이고, 조인키가 사업자번호가 아니라 고객코드다.** `SO_해외`의
  ...
  거래명세표와 동일하다
- **CI·PL의 HS CODE는 라인별로 찍는다 — 거래처 불문** (2026-09-04 양식 통일 지시).
  원래는 문서당 하나(`H12`/`I12`)였는데, SECTORIEL이 자기네 수입통관 코드를 라인마다
  요구하면서 판이 둘로 갈렸다. 갈래가 둘이면 "이 문서는 어느 규칙이지"를 매번 따져야
  하므로 하나로 합쳤다 — 모든 CI/PL이 D열에 라인별 HS를 싣고 헤더는 비운다.
  - 거래처 고유 코드를 받지 않은 문서는 **우리 수출신고 코드**(`config.DEFAULT_HS_CODE`
    = `8481.90.0000`, 헤더에 박혀 있던 값 그대로)를 물품 줄에 일괄로 넣는다.
    **운임 줄은 여기서도 비운다** — 물품이 아니라 HS가 애초에 없다
  - 표기가 문서마다 다른 게 정상이다: 기본값은 10자리 점 구분(우리 관세청 신고 형식),
    SECTORIEL 코드는 8자리(EU 형식). D 폭은 그 문서에 실릴 **가장 긴 코드**로 잰다
- **SECTORIEL은 CI·PL을 2장씩 낸다 — 수출신고용과 고객 제출용**
  (`config.HS_LINE_CUSTOMERS`, 고객명 부분일치 — 2026-09-02 사용자 지시).
  두 장의 **양식은 완전히 같고 D열 값만 다르다** (기본값 vs 고객 코드).
  `CI-HS_*`/`PL-HS_*`가 고객 제출용이다. 판정은 `hs_code.py`가 소유하고
...
- **CI/PL 양식은 템플릿 파일이 소유한다** (2026-09-07). 품목명 병합이 `A:C`이고 `D18`이
  `HS CODE`이며 문서 단위 HS(`H12`/`I12`)는 **비어 있다** — 한 장이 서로 다른 두 HS를
  주장하면 통관에서 어느 쪽을 믿을지 알 수 없다. 구 양식은 `templates/Old/`에 있다.
  생성기는 값만 쓰고, 값 쓰기 전에 `excel_helpers.assert_hs_template()`으로 **구 양식
  혼입을 막는다** — 구 양식이면 D에 쓴 코드가 병합 셀에 삼켜져 조용히 사라진다.
  같은 이유로 `layout_item_rows(hs_column=...)`는 **데이터가 아니라 템플릿을 따라간다**
  (CI/PL은 무조건 `True`) — `_hs_code` 유무로 정하면 그 열이 빠진 경로에서 `A:D`로
  재병합해 HS 열을 삼킨다.
  (예전엔 `apply_hs_layout()`이 생성할 때마다 임시 사본을 고쳤다. 판이 둘일 때는 템플릿을
  복제하면 갈라지니 그럴 만했는데, 양식이 하나로 통일되면서 근거가 사라졌다.)
  **아래 폭 이야기는 왜 템플릿이 지금 모양인지의 근거다** — 템플릿을 손볼 때 지킬 것.
  **품목명 폭(A:C)은 구 양식 A:D와 같고, 나머지 열은 한 칸도 건드리지 않았다.**
  그냥 떼면 200줄이 406 → 505줄(+24%)로 늘어 SECTORIEL 두 장의 쪽수가 갈린다(실측).
  폭 숫자는 코드에 박지 않는다: COM `ColumnWidth`와 파일의 width가 열 패딩(0.83자)만큼
  다르고, 병합 칸은 걸친 열 수만큼 패딩이 더 붙는다 — **시트를 재서** 맞춘다.
  **초과분(약 12자)은 인쇄 배율(`FitToPagesWide=1` → 실측 89%)로 흡수한다.**
  오른쪽 블록에서 폭을 걷어 보려다 두 번 사고가 났다: 폭 비례로 걷으면 헤더가 긴 열이
  잘리고(`Quantity`→`Quantit`, `Gross Weight` 2줄 클립), 열마다 자동 맞춤을 시키면
  **값을 쓰기 전이라** `PO No.`가 헤더 기준으로 줄어 긴 고객 PO가 잘렸다(반대로 위 헤더의
  `G16`은 복수 PO 결합이라 G를 89자로 부풀렸다). 짜낼 수 있는 건 3~5자인데 필요한 건
  12자라 얻는 것도 없었다. 배율로 흡수하면 열 폭이 그대로라 **줄바꿈·행 높이가 원래
  서식과 완전히 같다** (실측: 36행 높이 합 1090.05pt 동일)
- **거래명세표에 호선명을 표기하는 건 조선 기자재 4사뿐이다**
  ...
  (1) 하단 PO No.에 발주번호별 괄호 병기 `SCT2605-134 (한화-H4394, ...)`,
  (2) 본문 아이템 행의 비고(C열)에 행별 호선명 — 한 문서에 배가 여럿이면(실측
  DND-2026-0790: 12행에 3척) 하단 나열만으로는 어느 행이 어느 배인지 모른다.
  비고 우선순위는 월합 행별 PO > '선수금' > 호선명 (`build_row_remark` — 기존 표기를
  밀어내지 않는다). 다른 거래처는 Remarks에 내부 메모가 있어 붙이면 안 된다.
  **'Remarks'라는 이름이 시트마다 뜻이 다르다** — DN_국내 것은 월합 세금계산서 문구라,
  `load_dn_data()`가 SO 쪽을 `SO Remarks`로 개명해 병합하고 `ts_generator.build_po_cell_text()`가
  경로별 컬럼(DN=`SO Remarks`, 선수금=`Remarks`)을 명시로 고른다. 컬럼 유무로 추측하면
  월합 문구가 발주번호 옆에 샌다 (`tests/test_ts_generator.py`가 감시)
...
- **DN 라인은 `SO_국내`에 살아 있는 라인만이다** — PO를 그대로 옮기면 안 된다.
  ...
  **`세금계산서 발행일`은 채우지 않는다 — 사람이 직접 입력하는 칸이다** (2026-08-07 사용자
  지시). 발행 시점은 출고와 별개고(월합 거래처는 월말에 끊는다) 이 날짜의 '월'이 Order Book
  매출 인식월을 정한다 — 공란이면 미인식(Backlog 잔류)이라 잘못된 월에 매출이 잡히지 않는다.
  다만 **월합 거래처**는 Remarks를 물려줘 나중에 발행일을 채울 때 어느 날짜를 쓸지 알 수
  있게 한다. 문구의 출처는 **`Customer_국내.REMARK`(월합 키워드 매칭분)가 우선**이고 기존 DN
  최빈값은 폴백 — DN 이력에서 유도하면 거래처 약정이 바뀌었을 때 낡은 문구가 새 행마다
  복제된다 (2026-08-24 실측: 스칸텍 마스터는 '월 25일 마감, 말일자 세금계산서'인데 기존 DN의
  '25일 마감, 월합세금계산서'가 `DND-2026-0788~0790` 16행에 딸려왔다).
  ...
  **뒤 회차가 앞 회차 수량에 가려 통째로 사라진다** (2026-07 `SOD-2026-0713`: 7/23 L1 1개를
  ...
  그리고 똑같이 생긴 기록이 **다른 회차의 것**일 수 있다 — 균등 분할출고(40 = 20 + 20)는
  뒤 회차가 앞 회차와 라인·수량까지 같다 (2026-09 `SOD-2026-0868`: 9/3 기록 때문에 9/11
  출고가 '이미 입력됨(다른 출고일)'로 빠졌다). 그래서 **`PO_국내` Status로 가른다**
  (`_billed_beyond_dn`) — 이번 기간까지 `Invoiced`된 수량 − DN 수량에 이 출고가 라인마다
  다 들어가면 아직 안 적힌 출고다. 이 대조는 실행 중 추가분까지 센다('있는가'가 아니라
  '몫이 남았는가'라서). **SO 잔량으로 가르면 안 된다** — SO엔 아직 안 나간 회차까지 들어 있어
  날짜만 다르게 적은 부분 출고가 '자리가 있다'로 보여 두 번 들어간다.
  출고리스트는 같은 주문·같은 날을 두 줄로 적기도 해서(`SOD-2026-0467`) 이벤트를
  `(SO_ID, 출고일)`로 합친 뒤 순회한다
- **읽고 쓰는 CLI는 읽기 핸들을 넘기지 않는다** (`dn_recorder.load_source_frames`).
  `pd.ExcelFile`은 닫을 때까지 파일을 붙잡는다. 그걸 열어 둔 채 Excel에게 같은 파일을
  열라고 하면 **우리 프로세스가 우리를 막는다** — Excel이 `ReadOnly=True`로 열거나
  아예 열기에 실패한다. 2026-08-07 실사용에서 이 순서로 두 번 다 터졌다
  (`with pd.ExcelFile(...)` 한 줄이 없어서). `tests/test_dn_recorder.py`가 읽은 뒤
  파일을 rename해 보는 것으로 감시한다 (Windows는 열린 핸들이 있으면 rename을 막는다)
- **읽기 전용으로 열린 워크북은 조용히 저장에 실패한다** (`dn_writer.py`).
  `ReadOnly=True`에서도 **쓰기는 메모리에서 멀쩡히 되고 표 범위도 늘어나서**
  코드가 `A1:T1436`을 읽고 "4행 추가 완료"라고 보고하는데, `Save()`는 예외도 없이 무시되고
  파일은 그대로다 (`DisplayAlerts=False`면 '다른 이름으로 저장' 대화상자도 안 뜬다 —
  2026-08-07 실측·재현). 그래서 (1) `ReadOnly`를 확인해 **쓰기 전에** 막고,
  (2) 저장 뒤 파일의 mtime·size가 실제로 움직였는지 확인한다. 마스터 파일에 쓰는 경로는
  "COM이 예외를 안 냈다"를 성공으로 치면 안 된다
- **순서가 규약이다: 닫혀 있는지 확인 → 백업 → Excel 열기** (`dn_writer.append_lines`).
  **Excel이 워크북을 열면 읽기조차 막힌다**(실측: `open(path,'r+b')`도 `shutil.copy2`도
  PermissionError). 그래서 Excel을 띄운 뒤에는 백업 사본을 뜰 수 없다 — 되돌릴 곳 없이
  마스터를 건드리게 된다. 워크북이 이미 열려 있으면 붙어서 쓰지 않고 **닫고 오라고 한다**.
  누가 잡고 있는지는 COM을 띄우기 전에 `open(path, 'r+b')`로 판정한다
