...
- **읽기 전용으로 열린 워크북은 조용히 저장에 실패한다** (`dn_writer.py`).
  파일이 이미 Excel에 열려 있으면(본인 PC든, 공유라 다른 PC든) 두 번째 인스턴스는
  `ReadOnly=True`로 연다. 그 상태에서도 **쓰기는 메모리에서 멀쩡히 되고 표 범위도 늘어나서**
  코드가 `A1:T1436`을 읽고 "4행 추가 완료"라고 보고하는데, `Save()`는 예외도 없이 무시되고
  파일은 그대로다 (`DisplayAlerts=False`면 '다른 이름으로 저장' 대화상자도 안 뜬다 —
  2026-08-07 실측·재현). 그래서 (1) 열자마자 `ReadOnly`를 확인해 **쓰기 전에** 막고,
