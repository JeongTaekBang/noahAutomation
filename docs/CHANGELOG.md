...
## 2026-08-07: DN 쓰기 — 읽기 전용 워크북에서 조용히 실패하던 것 수정

`create_dn.py P08`이 "완료: 4행 추가 (DN_국내 1433~1436행, 표 범위 A1:T1436)"이라고
보고했는데 **파일에는 아무것도 안 들어갔다.**

원인: 워크북이 이미 Excel에 열려 있으면 두 번째 인스턴스가 `ReadOnly=True`로 연다.
그 상태에서도 **쓰기는 메모리에서 멀쩡히 되고 표 범위도 늘어나서** 코드가 `A1:T1436`을
읽고 성공이라고 판단하는데, `Save()`는 예외 없이 무시된다
(`DisplayAlerts=False`라 '다른 이름으로 저장' 대화상자도 안 뜬다). 사본으로 재현 확인.
이 파일은 OneDrive 공유라 **본인이 안 열어 뒀어도 다른 PC에서 열려 있으면** 읽기 전용이 된다.

- 열자마자 `ReadOnly` 확인 → **쓰기를 시작하기 전에** 중단하고 무엇을 해야 하는지 알린다
- 저장 뒤 파일의 mtime·size가 실제로 움직였는지 확인 → 안 움직였으면 실패로 보고
- 백업은 `ReadOnly` 확인 **뒤에** 뜬다 — 못 쓸 걸 알면서 5MB 사본을 OneDrive에 쌓지 않는다
- 열기·저장이 COM 예외로 죽는 경우도 원문 대신 **행동할 수 있는 안내**로 바꾼다
  (`_busy_message`). Excel은 둘 다 같은 문구('cannot access the file' + 가능성 3가지)로
  돌려줘서 그대로 보여 주면 무엇을 해야 하는지 알 수 없다 — 실제 원인은 거의 항상
  "누가 잡고 있다"(Excel 창 / 다른 PC / OneDrive 동기화)이므로 그걸 앞에 세운다
- `tests/test_dn_writer.py` 신규 10건 — COM 없이 워크북을 흉내 내
  "쓰기가 실패했는데 성공이라고 보고하는" 경로가 없는지 본다

교훈: 마스터 파일에 쓰는 경로에서 **"COM이 예외를 안 냈다"를 성공으로 치면 안 된다.**
쓴 결과를 되읽어 확인해야 한다.
