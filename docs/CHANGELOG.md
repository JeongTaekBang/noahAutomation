...
## 2026-09-11: DN — 균등 분할출고의 뒤 회차가 '이미 입력됨'으로 빠지던 것 수정

`SOD-2026-0868`(ND-0877): 40개 한 라인을 공장이 20 + 20으로 나눠 9/3·9/11에 출고했다
(`PO_국내`에 같은 Line item 1로 행이 하나 더 생김). 9/3분이 DN에 기록된 뒤
`create_dn.py P09`를 돌리면 9/11분이 "추가 대상 없음"으로 끝났다.

원인은 중복 방어 "라인·수량이 똑같은 출고가 다른 날짜로 이미 기록됨"이다. 사람이 출고일을
다르게 적어 둔 건을 잡으려는 규칙인데, **균등 분할출고는 뒤 회차가 앞 회차와 라인·수량까지
같아서** 9/3 기록을 9/11의 것으로 착각했다. 그 기록이 **이미 다른 출고의 것인지**를 보지
않은 것이 결함이다.

수정: 똑같이 생긴 기록이 있어도 **공장 계산서가 DN보다 이 출고만큼 앞서 있으면** 넣는다
(`_billed_beyond_dn`). 공장은 분할출고마다 `PO_국내` 행을 따로 두고 `Invoiced P{XX}`를
적으므로, 이번 기간까지 계산서가 끊긴 수량 − DN 수량에 이 출고가 라인마다 다 들어가면 DN에
없는 출고가 있다는 뜻이다 (0868: 계산서 40 − DN 20 = 20 ≥ 20).

- **달을 넘긴 균등 분할**도 같은 규칙으로 풀린다 — 이 주문의 원래 계획이 9/3 + 10/6이었다.
  다음 달 출고리스트엔 앞 회차가 없어서 "그 기록은 다른 출고의 것"이라고 말해 줄 근거가
  PO Status뿐이다
- **SO 잔량으로 가르지 않은 이유**: SO엔 아직 안 나간 회차까지 들어 있어, 날짜만 다르게 적은
  부분 출고(20 출고 · 20 대기)가 '자리가 있다'로 보여 두 번 들어간다. PO 계산서는 20뿐이라 막힌다
- PO에 따로 없는 라인(부속)은 근거가 없어 판단에서 빼고, 판단할 라인이 없으면 기존대로 건너뛴다
- 곁들여 코드 주석의 `SOD-2026-0156` 날짜 오기(DN 5/19 → 실제 3/9)를 바로잡았다

### 검증
- 테스트 4건 추가 — 같은 달 / 달 넘김(마지막·중간 회차) / 날짜만 다르게 적은 부분 출고는
  여전히 건너뜀. 앞의 3건이 수정 전 코드에서 실패하는 것을 먼저 확인했다.
  전체 **909 passed, 2 skipped**
- 실데이터 수정 전/후 대조 (P03~P09, 옛 모듈을 `git show`로 떠서 같은 입력에 나란히 실행):
  - 기간 전체 실행(실제 DN 상태): 달라진 건 P09 `SOD-2026-0868` 9/11 L1 20개 **1건뿐**.
    이 규칙이 원래 잡던 4건(0156·0374·0429·0876 — 전부 출고리스트 어느 기간에도 없는 날짜로
    적힌 기록)은 그대로 건너뛴다
  - 출고 이벤트별 증분 재현 658건: 수정 전/후 결과가 다른 이벤트 **0건**
