...
**양식은 템플릿 파일이 소유한다** (2026-09-07) — 생성기는 값만 쓴다.
구 양식은 `templates/Old/`에 있다.

| 위치 | 구 양식 (`Old/`) | 지금 템플릿 |
|------|------------------|-------------|
| 품목명 병합 (헤더~Total 행) | `A:D` | **`A:C`** (D를 HS 열로 내줌) |
| `D18` | 품목명 병합 안 | **`HS CODE`** |
| `D20`~ | 품목명 병합 안 | **HS 코드 자리** (텍스트 서식 `@`, 가운데 정렬) |
| `H12`/`I12` | `HS CODE: ` / `8481.90.0000` | **공란** (문서 단위 HS가 라인별과 충돌) |
| C 열 폭 | 13.83 | D가 가져간 만큼 되돌림 (**품목명 폭 = 구 양식 A:D**) |
| D 열 폭 | 7.33 / 6.66 | 가장 긴 표기(`8481.90.0000`) 기준 — CI 14.5 / PL 14.33 |
| E~I 열 폭 | — | **구 양식 그대로** |
| 인쇄 배율 | 없음 | **`FitToPagesWide = 1`** (초과분 흡수, 실측 89%) |

생성기는 값을 쓰기 전에 `excel_helpers.assert_hs_template()`으로 **구 양식 혼입을 막는다**
— 구 양식이면 D에 쓴 코드가 병합 셀에 삼켜져 조용히 사라진다.
`tests/test_doc_layout.py: TestHsTemplateShape`가 템플릿 실물을 감시한다.
