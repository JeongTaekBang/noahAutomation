...
let
    ...
    // 해외: 공통컬럼 + Sales amount KRW
    SO_해외_Selected = Table.SelectColumns(SO_해외_Raw, CommonColumns_Filtered & {"Sales amount KRW", "Status"}),
...
SO 시트:
┌───────────┬───────┬──────┬─────────┬────────┬──────────┐
│ SO_ID     │ Line  │ OS   │ Period  │ qty    │ amount   │
├───────────┼───────┼──────┼─────────┼────────┼──────────┤
│ SOD-0001  │ 1     │ IQ3  │ 2026-01 │ 10     │ 500만    │
│ SOD-0001  │ 2     │ IQ3  │ 2026-01 │ 5      │ 250만    │
└───────────┴───────┴──────┴─────────┴────────┴──────────┘
...
└───────────┴───────┴──────┴─────────┴────────┴──────────┴──────────┘
             출고월 = 마지막 출고 시점 (ActivePeriods 범위 결정용)
```

**② Period 확장** (등록월 ~ 마지막출고월)
```
┌───────────┬───────┬──────┬──────────┬─────────┐
│ SO_ID     │ Line  │ OS   │ 등록P    │ Period  │  ← 확장된 Period
├───────────┼───────┼──────┼──────────┼─────────┤
│ SOD-0001  │ 1     │ IQ3  │ 2026-01  │ 2026-01 │  ← 등록~
│ SOD-0001  │ 1     │ IQ3  │ 2026-01  │ 2026-02 │  ←      ~출고월
│ SOD-0001  │ 2     │ IQ3  │ 2026-01  │ 2026-01 │  ← 1월만 (1월 출고완료)
└───────────┴───────┴──────┴──────────┴─────────┘
```

**③ Input 계산** (Period = 등록Period일 때만)
```
┌───────────┬───────┬─────────┬───────────┬──────────────┐
│ SO_ID     │ Line  │ Period  │ Input_qty │ Input_amount │
├───────────┼───────┼─────────┼───────────┼──────────────┤
│ SOD-0001  │ 1     │ 2026-01 │ ★ 10      │ ★ 500만      │  ← 등록월 = Input
│ SOD-0001  │ 1     │ 2026-02 │ 0         │ 0            │
│ SOD-0001  │ 2     │ 2026-01 │ ★ 5       │ ★ 250만      │  ← 등록월 = Input
└───────────┴───────┴─────────┴───────────┴──────────────┘
```

**④ Output 조인** (DN_ByMonth와 Period = 출고월 매칭)
```
DN_ByMonth:                              매칭
┌──────────┬──────┬─────────┬─────┐     ┌─────────────────────────────────────────────┐
...
**⑤ OS name 그룹화** (SO_ID + IQ3 + Period → Line item 합산)
```
┌───────────┬──────┬─────────┬───────────┬────────────┐
│ SO_ID     │ OS   │ Period  │ Input_qty │ Output_qty │
├───────────┼──────┼─────────┼───────────┼────────────┤
│ SOD-0001  │ IQ3  │ 2026-01 │ 10+5 = 15 │ 3+5 = 8    │  ← Line 1+2 합산
│ SOD-0001  │ IQ3  │ 2026-02 │ 0         │ 7          │
└───────────┴──────┴─────────┴───────────┴────────────┘
```

**⑥ 롤링 계산** (Start → Ending 전파)
```
┌─────────┬───────┬────────┬────────┬──────────┐
│ Period  │ Start │ Input  │ Output │ Ending   │
├─────────┼───────┼────────┼────────┼──────────┤
│ 2026-01 │ 0     │ +15    │ -8     │ = 7      │  ← Backlog 7
│ 2026-02 │ 7 ◀───│────────│────────│ (이월)    │
│         │       │ +0     │ -7     │ = 0      │  ← 소진
└─────────┴───────┴────────┴────────┴──────────┘
                                      ↑
                               P01 Ending = P02 Start
```
