...
2. **입력 효율성**: PO는 SO를 `=` 셀 참조로 가져옴
3. **행 동기화**: SO와 PO는 같은 행 번호 유지 (1:1 매칭)
4. **실수 방지**: 행 삭제 잠금 설정
...
**SO_ID 규칙**:
- 형식: `SO-YYYY-NNNN` (예: SO-2026-0001)
- 같은 주문의 여러 품목은 **같은 SO_ID** 사용
- 피벗테이블로 SO_ID 기준 그룹핑 가능

**예시**:
```
┌──────────────┬──────────┬────────┬───────┬──────┐
│ SO_ID        │ 고객PO   │ 고객명 │ 모델  │ 수량 │
├──────────────┼──────────┼────────┼───────┼──────┤
│ SO-2026-0001 │ ABC-123  │ A사    │ IQ10  │ 10   │
│ SO-2026-0001 │ ABC-123  │ A사    │ IQ18  │ 20   │
│ SO-2026-0001 │ ABC-123  │ A사    │ IQ25  │ 5    │
│ SO-2026-0002 │ DEF-456  │ B사    │ NA038 │ 100  │
└──────────────┴──────────┴────────┴───────┴──────┘
```
...
**입력 방식**:
- SO 정보는 `=SO_국내!A2` 형태로 **셀 참조**
- 사양/옵션만 직접 입력
- **SO와 행 번호 1:1 동기화** (SO 2행 = PO 2행)

**컬럼 구성**:

| 구분 | 컬럼 | 입력 방식 |
|------|------|-----------|
| SO 참조 | SO_ID, 고객명, 모델, 수량 등 | `=SO_국내!xx` 참조 |
...
```
┌──────────────┬────────┬───────┬───────────┬───────────┬─────────────┐
│ SO_ID        │ 고객명 │ 모델  │ RCK Order │ 발주일    │ 사양/옵션...│
│ (=SO 참조)   │(=참조) │(=참조)│ (직접)    │ (직접)    │ (직접)      │
├──────────────┼────────┼───────┼───────────┼───────────┼─────────────┤
│ SO-2026-0001 │ A사    │ IQ10  │ ND-0001   │ 2026-01-15│ ...         │
│ SO-2026-0001 │ A사    │ IQ18  │ ND-0001   │ 2026-01-15│ ...         │
│ SO-2026-0001 │ A사    │ IQ25  │ ND-0001   │ 2026-01-15│ ...         │
└──────────────┴────────┴───────┴───────────┴───────────┴─────────────┘
```
...
│  SO_국내 (Sales Order)                                              │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │ SO_ID (PK) │ 고객PO │ 고객명 │ 모델 │ 수량 │ 금액 │ 납기 │ ...│   │
│  └─────────────────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────────────────┘
        │                               │                    │
        │ 1:1 (행 번호 동기화)          │ 1:N               │ 1:N
        ▼                               ▼                    ▼
┌───────────────────┐         ┌─────────────────┐    ┌─────────────────┐
│  PO_국내          │         │  DN_국내        │    │  PMT_국내       │
│  (Purchase Order) │         │  (Delivery)     │    │  (Payment)      │
├───────────────────┤         ├─────────────────┤    ├─────────────────┤
│ =SO 참조          │         │ SO_ID (FK)      │    │ SO_ID (FK)      │
│ + 사양/옵션       │         │ 납품일, 수량    │    │ 구분 (선수금 등)│
│                   │         │ 세금계산서      │    │ 금액, 입금일    │
└───────────────────┘         └─────────────────┘    └─────────────────┘
```
...
- **행 삭제: 금지**
- 셀 편집: 허용
- 이유: SO/PO 행 번호 동기화 유지

**DN_국내, PMT_국내**:
- 행 삽입/삭제: 허용
- 이유: SO_ID로 연결되므로 행 번호 무관
...
- [ ] 엑셀 시트 구조 생성 (수동)
- [ ] 각 시트 표(Table)로 변환 (Ctrl+T)
- [ ] 파워쿼리로 SO_header 생성 (중복 제거)
- [ ] 파워피벗 관계 설정
- [ ] 기존 데이터 마이그레이션
- [ ] 시트 보호 설정 (SO/PO 행 삭제 금지)
