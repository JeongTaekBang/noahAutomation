...
| `--date` | TS | 그날 출고분을 자동으로 골라 **거래처별 메일 1통**으로 (예: `--date 2026-08-06`) |
| `--customer` | TS | `--date` 안에서 거래처 한 곳만 (사업자번호 또는 거래처명 일부) |
| `--one-mail` | TS | 문서는 DN별 1장 그대로, **메일만** 거래처별 한 통(첨부 여러 개)으로 |
...
`create_po.bat` 더블클릭 → 대화형 메뉴에서 문서 유형 선택 (PO, TS, PI, FI, OC)

거래명세표는 하위 메뉴가 넷입니다:

| 메뉴 | 하는 일 |
|------|---------|
| [1] 단건 | DN 1건 → 문서 1장, 메일 1통 |
| [2] 월합 | 여러 DN을 **문서 한 장**으로 합침 |
| [3] 하루치 묶음 발송 | 출고일(+거래처)로 골라 문서는 DN별 1장, **메일은 거래처별 1통**(첨부 여러 개). 거래처를 지정하면 발송 전 y/N을 한 번 묻고, **비워 두면(그날 전체) 확인 없이 초안**을 띄운다 — 거래처마다 묻지 않으려고 |
| [4] 묶음 발송 | DN 목록을 붙여넣어 [3]과 같은 방식으로 |

[2]는 '문서'를 합치고 [3][4]는 '메일'만 합칩니다 — 같은 날 한 거래처로 여러 PO가
나갔을 때 명세표는 PO별로 따로 주되 메일은 한 통으로 보내려면 [3]입니다.
