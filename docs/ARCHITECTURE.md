...
├── 📄 dashboard.py          # Streamlit 대시보드 (수주/출고/제품/섹터/고객/백로그)
│
├── 📁 po_generator/         # ⭐ 핵심 패키지
...
└─────────────────────────────────────────────────────────────────────────┘
  ...
  ┌─────────────────────────────────────────────┐
  │ FinderService (인스턴스 레벨)                │     프로세스 수명
  │  _po_df, _dn_df, _pmt_df, _so_domestic_df  │     (lazy loading)
  │  _so_export_df, _so_export_cust_df          │
  │  _dn_export_df                              │
  │                                             │
  │  → load_*() 최초 호출 시 Excel 로드         │
  │  → 이후 호출은 캐시 반환 (I/O 없음)         │
  └─────────────────────────────────────────────┘

  ┌─────────────────────────────────────────────┐
  │ resolve_column() 캐시 (모듈 레벨)            │     프로세스 수명
  │  _resolve_cache: dict[(id(columns), key)]   │
  │                                             │
  │  → get_value() 매 호출 시 별칭 해석 캐시     │
  │  → 동일 DataFrame에서 반복 조회 O(1)         │
  └─────────────────────────────────────────────┘

  ┌─────────────────────────────────────────────┐
  │ get_spec_option_fields() 캐시 (모듈 레벨)    │     프로세스 수명
  │  _spec_option_fields_cache: dict[시트타입]   │
  │                                             │
  │  → PO 시트 컬럼 구조 캐시                    │
  │  → 동적 SPEC/OPTION 필드 목록               │
  └─────────────────────────────────────────────┘

get_available_*_ids() 최적화:
  Before: unique() 루프 × df[df[col]==id] 필터 → O(n²)
  After:  drop_duplicates().head(limit) 단일 패스  → O(n)
```
