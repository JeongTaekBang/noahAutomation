...
-- Output(매출월·금액)과 환율 재평가분은 `v_dn_revenue` 뷰가 단일 정의 (db_schema.py)
-- 잔고 = Input − Output + Variance → 환율차가 상쇄되어 재환산 도입 전과 같은 값
...
events_line_item AS (
    SELECT s.SO_ID, s.[Customer name], s.[Customer PO], s.[Item name],
        ...
        0 AS Value_Output_qty, 0 AS Value_Output_amount, 0 AS Value_Variance_amount
    FROM so_combined s
    UNION ALL
    SELECT dm.SO_ID,
...
backlog AS (
    SELECT
        ...
        SUM(Value_Input_amount - Value_Output_amount + Value_Variance_amount) AS Value_Ending_amount
    ...
    HAVING ABS(SUM(Value_Input_qty - Value_Output_qty)) > 0.001
        OR SUM(Value_Input_amount - Value_Output_amount + Value_Variance_amount) > 0.5
)
