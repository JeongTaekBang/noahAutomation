"""
Excel Generator 테스트 (xlwings 기반)
=====================================

excel_generator.py 모듈의 테스트
xlwings로 생성된 PO 파일을 openpyxl로 검증
"""
...
from po_generator.template_engine import generate_po_template
...
def save_and_load_workbook(wb, sheet_name: str = "Purchase Order"):
    """POWorkbook을 저장하고 openpyxl로 로드하여 검증"""
    with tempfile.NamedTemporaryFile(suffix='.xlsx', delete=False) as f:
        temp_path = Path(f.name)

    wb.save(temp_path)
    loaded_wb = load_workbook(temp_path)
    ws = loaded_wb[sheet_name]
    return loaded_wb, ws, temp_path
...
class TestCreatePurchaseOrder:
    ...
    def test_creates_sheet_with_correct_title(self, valid_order_data):
        ...
        loaded_wb, ws, temp_path = save_and_load_workbook(wb)
    ...
    def test_delivery_address_from_column(self):
        ...
        wb = create_po_workbook(order_data)
        loaded_wb, ws, temp_path = save_and_load_workbook(wb)
...
class TestCreateDescriptionSheet:
    ...
    def test_creates_line_no_header(self, valid_order_data):
        ...
        wb = create_po_workbook(valid_order_data)
        loaded_wb, ws, temp_path = save_and_load_workbook(wb, "Description")
...
class TestCreatePoWorkbook:
    ...
    def test_creates_both_sheets(self, valid_order_data):
        ...
        try:
            wb.save(temp_path)
            loaded_wb = load_workbook(temp_path)
            assert "Purchase Order" in loaded_wb.sheetnames
            assert "Description" in loaded_wb.sheetnames
            loaded_wb.close()
        finally:
            temp_path.unlink()
