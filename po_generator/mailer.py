...
def wrap_body_html(inner: str) -> str:
    ...
    실측). 본문 전체를 `<table><tr><td>` 한 칸에 담으면 서명이 **인사말 앞으로 끼어들지는
    않는다**. 메일은 어차피 끝에 "자동 발송" 안내를 달고 나가므로 서명 없는 쪽이 낫다는
    운영 결정이고, 거래명세표·납기현황이 같은 래퍼를 써서 동작이 갈리지 않는다.

    **다만 이 구조가 서명을 없애 주지는 않는다** (2026-09-10 실측). 2026-07-29에는 새
    Outlook이 이 구조에 서명을 아예 안 붙였지만 클라이언트가 갱신되며 다시 붙는다 —
    억제는 Outlook의 관찰된 동작에 기댄 것이라 수명이 있다. 그래서 서명 유무는 **Outlook
    설정 쪽**에서 정한다 (새 Outlook → 설정 → 계정 → 서명 → '새 메시지' = (없음),
    2026-09-10 사용자 결정). 코드로 확실히 없애려면 작성창(인스펙터)을 아예 만들지 않는
    경로여야 한다 — COM `Save()`로 임시보관함에 넣거나 `--send` 즉시 발송.
