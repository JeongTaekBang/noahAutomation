...
자동으로 Packing List를 생성합니다.

사용법:
    python create_pl.py DNO-2026-0001              # 단일 생성
    python create_pl.py DNO-2026-0001 DNO-2026-0002 # 여러 건 동시 생성
"""
...
def print_available_ids(df_dn: pd.DataFrame, limit: int = 15) -> None:
    ...
    print(f"\n[Packing List] DN_ID ({len(dn_ids)}건)")
    print("-" * 40)
    ...
    print("\n" + "=" * 60)
    print("위 DN_ID 중 하나를 입력하여 Packing List를 생성하세요.")
    print("=" * 60)
...
def generate_pl(dn_id: str, df_dn: pd.DataFrame) -> bool:
    ...
    # 3. 문서 생성 (서비스 사용)
    result = service.generate_pl(dn_id)
...
Packing List를 자동 생성합니다.
"""

    epilog = """
사용 예시:
  python create_pl.py DNO-2026-0001              # PL 1건 생성
  python create_pl.py DNO-2026-0001 DNO-2026-0002 # 여러 건 동시 생성
